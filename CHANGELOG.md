...
- New `y-grid` component — a layout container that arranges its children using CSS Grid, with optional masonry layout (extracted from `y-stack` to give grid/masonry a dedicated component). Exposes the most common CSS Grid configuration as HTML attributes: `mode` (`"grid"` | `"masonry"`), `columns` (integer, `"auto"`, or raw `grid-template-columns`), `rows`, `auto-flow`, `auto-rows`, `auto-columns`, `gap`/`row-gap`/`column-gap` (mapped to `--spacing-*` tokens), `align`, `justify`, `align-content`, `justify-content`, `min-item-width`, `responsive`, and `dense`. Children may use inline `grid-column`/`grid-row` styles to span tracks. Masonry mode positions items absolutely into the shortest column via JS using a `ResizeObserver` and rAF-batched relayouts, and emits `y-grid-layout` (`{ mode, columns, containerWidth }`) after each settle. Responsive collapse (integer `columns` only) is mode-specific: grid mode uses `repeat(auto-fit, minmax(...))` keyed off `min-item-width`, while masonry mode is breakpoint-driven via configurable mobile/tablet tokens.
