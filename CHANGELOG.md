...
- New `y-break` component — a decorative divider that draws a horizontal or vertical line, optionally broken by centered content. Attributes: `orientation` (`horizontal` default | `vertical` — host needs a height when vertical), `align` (`start` | `center` default | `end`), `variant` (`solid` default | `dashed` | `dotted`), `label` and `icon` convenience attributes for centered text/icon, and `inset` (`none` default | `sm` | `md` | `lg`) for outer-end padding. Default slot accepts arbitrary content (badges, buttons, etc.) and takes precedence over `label`/`icon`. Exposes `line`, `line-start`, `line-end`, `content` parts, and a full set of `--component-break-*` custom properties (including `--component-break-line-style` for per-instance variant overrides). Host gets `role="separator"` plus `aria-orientation="vertical"` when vertical.
